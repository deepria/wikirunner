...
- 웹 사용자가 플레이어를 생성하고, 5분 만료 일회용 코드를 발급한다.
- 확장 프로그램은 코드 교환 후 같은 플레이어의 `extension` identity가 된다.
- 페어링 코드 원문은 저장하지 않고 서버 비밀값으로 만든 해시만 저장한다.
...
| `players` | 방 안의 닉네임, 준비·연결 상태와 확장 연결 시각 |
...
| `violations` | 이동 검증 및 검색·새 탭 공정 플레이 경고 |
...
6. 확장 프로그램은 해당 시각에 시작 문서를 전용 게임 탭에서 연다.
...
3. 쉬움 3~4, 보통 5~6, 어려움 7~8단계까지 반복하며 최대 8회 재시도한다.
...
확인된 클릭·단축키와 URL 패턴을 차단한다. 검색 시도는 `search_attempt`, 별도 나무위키
탭 이동은 `new_tab` 경고로 서버에 전송한다. 자동 실격은 하지 않는다.
...
| `POST /v1/games/:id/violations` | 확장 | 검색·새 탭 경고 보고 |
